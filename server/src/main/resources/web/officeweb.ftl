...
<#-- 在线协作编辑：presence 由 collab-stomp.js 统一处理（在线用户面板由其注入，
     SockJS/STOMP 库按需动态加载本地副本）。本页开头已把 console.log/warn 置为
     noop，公共组件的常规日志走 console.info、错误走 console.error，不受影响 -->
<#if collaborationEditEnabled!false>
    <link rel="stylesheet" href="/css/collab.css"/>
    <script src="/js/collab-stomp.js"></script>
    <script>
        /**
         * 处理远程协作操作：将远程单元格编辑应用到 Luckysheet。
         * 预留：后端操作通道尚未实现，当前不会收到该消息，
         * 传入 onOperation 仅用于保留接入点（有消息时才会订阅 /topic/session/{sessionId}）。
         * @param {{type?: string, cellRange?: {row: number, column: number}, content?: string}} operation 远程操作数据
         */
        ...
        initCollabStomp({
            ...
            enableCursor: false,
            onOperation: handleCollabOperation
        });
    </script>
</#if>
